...
@onready var tool_telling: Button = $MarginContainer/HBoxContainer/ToolTelling
...
func _ready() -> void:
	...
	tool_telling.disabled = true
	tool_telling.focus_mode = Control.FOCUS_NONE
...
func on_enable_tool_button(tool : DataTypes.Tools) -> void:
	if tool == DataTypes.Tools.TillGround:
		tool_telling.disabled = false
		tool_telling.focus_mode = Control.FOCUS_ALL
