...
@export var day_night_gardient_texture : GradientTexture1D
...
func _ready() -> void:
	...
	DayAndNightCycleManager.game_time.connect(on_game_tick)

func on_game_tick(time : float) -> void:
	var sample_value = 0.5 * (sin(time - PI * 0.5) + 1.0)
	color = day_night_gardient_texture.gradient.sample(sample_value)
